...
| BL-004 | Non-positive quantity rejected | `addToCart` and `updateQuantity` throw `ValidationError` when quantity is not a positive integer (`<= 0` or non-integer) (HTTP 400). |
...
| BL-012 | Cart − enabled at quantity 1 | Prototype UI: CartLine keeps the decrement control enabled when quantity is 1 so a further − calls the API and surfaces validation errors via ErrorBanner (see [ADR 0010](./decisions/0010-validation-and-error-handling.md)). |
