...
- **Layer responsibilities (backend):** Express routes/controllers are a thin HTTP layer (routing, request/response plumbing). Domain logic (cart calculations, add/increment/remove rules) lives in a separate domain layer. See [ADR 0004](./decisions/0004-server-framework.md).
