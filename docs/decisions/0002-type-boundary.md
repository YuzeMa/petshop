...
- **Decision:**
  - Share **only the REST request/response types** plus the structured error shape, via `packages/api-types` (plain TypeScript types for now; Zod schemas planned in backlog P2-8).
...
- **Consequences:**
  ...
  - The api-types package is the single source of truth for the wire format; runtime validation with Zod is planned (see backlog P2-8).
